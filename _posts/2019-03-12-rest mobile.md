...
플랫폼(sdk)이 쉽고 탄탄해야 하죠~

특히 메이저 플랫폼(google, apple, ms)들은 개발자 마다 다른 개성(코드 스파게티화)을 반 강제적인 패턴과 예외처리(옵셔널 델리게이트등)로 막아내고 있는 것 같습니다.
...
최대한 3rd파티 라이브러리 사용을 금해야하지만 필요할 경우 메인플랫폼을 따라가며 같이 갈 동반자(updated가 최소 2달 이내의 라이브러)를 찾아야합니다.
...
큰조직의 경우 외주 없이 직접 운영한다면 디비에 ?명 백엔드에 ?명 프론트에 ?명 모바일에 ?명 품질 ?명 엄청난 개발자들이 잘게 분산되어 일하고 있습니다.
...
적은 인원에 이것저것 하다보니 한사람이 백엔드+디비, 프론트+모바일, 백엔드+프론트+배포 등으로 자동 묶음 상황이 허다하게됩니다.(_한편으로는 기회?_)
...
개발자는 구글+스택오버플로 면 끝난다고들 합니다.

맞는 말이긴 한데 문제는 이해를 하지 않고 줏어온 코드들이 스파게티를 이루게 되는 상황을 많이 봤습니다.
...
그래서 당연히 제조사, 제작사의 개발자 데이터시트, 문서가 최고의 정답이 나와있습니다.

최고의 정답이긴 한데.. 대부분 영어인데다가 시작부터 데이터시트 보며 공부하면 너무 어렵다는 것이죠.

제일 좋은 방법이라 생각하는 것은

구글링 -> 스택오버 예제 -> 내 프로젝트에 적용(되는 것만 확인) -> 해당 내용(클래스등)을 공식 개발자 사이트에서 검색 -> 취합해서 최신화된 코드로 내 코드에 적용 

> 스위프트의 경우 구글 1년이내 검색을 해야합니다. 디프리케이트된 기능들이 너무 많습니다..  
공식 사이트에서 디프리케이트 된 기능이 뭘로 대체(instead) 되었는 지 확인하는 것이 필요합니다. 
...
윈도우의 경우 무려 일억번을 다시그립니다. CPU점유율 100%입니다(4코어면 25%)
...
> 깃헙 포트폴리오나 npm 모듈 등록 같은 행위가 몇100장 이력서나 자소서 학력보다 훨씬 우대가 됩니다.     
...
- 웹서버(node.js) :   
- ~~백엔드(express.js)~~ : node 대체
- 프론트(vue.js)
- 모바일

클라우드가 전문인력을 완전 대체하긴 어렵지만 그럭저럭 대체할 수준 입니다.

디비의 경우 나름 주기 스냅샷 롤백 같은 것을 지원해서 적당한 규모에서는 DBA보다 클라우드 프로등을 이용해서 지원(SLA)을 받는 것이 좋다고 생각합니다.
...
주의할 것은 파이어베이스 디비에 적당한 양만 가능합니다.(_읽고 쓰는 비용이 어느 수준을 넘어가면 장난 아닌 금액이 나옵니다._)
...
웹솔루션의 경우 소스코드 올려주고 디비 연결은 쓴다 읽는다 만 남는 것입니다. 
...
코틀린이 비약적으로 코드가 줄어드는 것을 확인할 수 있었습니다.
...
기록과 학습을 위해 모바일 강좌와 현재 프로젝트 진행을 같이하는 것이 목적이었는데 하다보니 꽂혀버려서 앱이 벌써 완성 되어버렸네요.
